function  results = pctCorrectTest( data, numPartitions )
...
testPctCorrect = cell(numPartitions, 1);
...
for i = 1:numPartitions
    ...
    [models{i}, ~] = pctModels(trainData, 25);
    [trainAccuracy, thresholds, trainrmses] = pctCorrectAllModels(models{i}, trainData);
    [testAccuracy, rmses, errorPerAttempt{i}, errorByResourceCount{i}, errorByTraceLength{i}, actualPctCorrect] = pctCorrectAllModels(models{i}, testData);
    ...
    testPctCorrect{i} = actualPctCorrect;
end
cverr = sum(errs);
...
results.testPctCorrect = testPctCorrect;
